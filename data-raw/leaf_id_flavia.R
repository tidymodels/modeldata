...
leaf_id_flavia <-
  ...
  mutate(
    ...
    edges = gsub("_$", "", edges),
    denate_edge = ifelse(edge_type == "denate" | edge_type_2 == "denate",
                         "yes", "no"),
    lobed_edge = ifelse(edge_type == "lobed" | edge_type_2 == "lobed",
                        "yes", "no"),
    smooth_edge = ifelse(edge_type == "smooth" | edge_type_2 == "smooth",
                         "yes", "no"),
    toothed_edge = ifelse(edge_type == "toothed" | edge_type_2 == "toothed",
                          "yes", "no"),
    undulate_edge = ifelse(edge_type == "undulate" | edge_type_2 == "undulate",
                           "yes", "no"),
  ...
  relocate(species, apex, base, shape, denate_edge, lobed_edge,
           smooth_edge, toothed_edge, undulate_edge)
